function Population = DeGuideOne(Problem, rRelate, flag, W, Z, eta,guideGEN)
...
%--------------------------------------------------------------------------
    ...
    Population = guidePop;     % 二阶段第一类向量初始种群保留下来
    len   = size(rRelate,2);
    wr = find(flag == 1);
    WG = W(wr,:);
    T = ceil(min(len,10));
    ...
    for i = 1 : len
        P = B(i,randperm(size(B,2)));
        Offspring = guidePop(i);
        Z = min(Z,Offspring.obj);
        normW   = sqrt(sum(WG(P,:).^2,2));
        normP   = sqrt(sum((Population(P).objs-repmat(Z,T,1)).^2,2));
        normO   = sqrt(sum((Offspring.obj-Z).^2,2));
        CosineP = sum((Population(P).objs-repmat(Z,T,1)).*WG(P,:),2)./normW./normP;
        CosineO = sum(repmat(Offspring.obj-Z,T,1).*WG(P,:),2)./normW./normO;
        g_old   = normP.*CosineP + 5*normP.*sqrt(1-CosineP.^2);
        g_new   = normO.*CosineO + 5*normO.*sqrt(1-CosineO.^2);
