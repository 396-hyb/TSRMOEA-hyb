function Population = Final(Problem,IndexArr,ObjsArch,DecsArch,ArchGEN,W,Z,eta,Population,guideGEN)
...
%--------------------------------------------------------------------------
    ...
    if ~isempty(rRelateV1)
        if length(find(flagV1 == 1)) == 1
            ...
            populationOne = DeGuideOne(Problem, rRelateV1, flagV1, W, Z, eta,guideGEN);
    ...
    if ~isempty(rRelateV2)
        if length(find(flagV2 == 1)) == 1
            ...
            populationTwo = DeGuideTwo(Problem, Population, rRelateV2, flagV2, W, Z, eta,guideGEN);
        end
    end
    ...
    if length(find(flagV3 == 1)) > 0
        if ~isempty(rRelateV3)
            if length(find(flagV3 == 1)) == 1
                ...
                RFE = RFE + size(rRelateV3,2)*100*50;
                populationThree = DeGuideThree(Problem, rRelateV3, flagV3, W, Z, eta,guideGEN);
                % populationThree = DeGuideThree(Problem,Arch{gen,:}, flagV3, W, Z, eta);
            end
        end
    end
