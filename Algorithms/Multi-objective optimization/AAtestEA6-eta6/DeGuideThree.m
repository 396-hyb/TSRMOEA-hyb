function Population = DeGuideThree(Problem, Population, rRelate, flag, W, Z, eta, remainGen)
...
%--------------------------------------------------------------------------
    ...
    for i = 1 : Vsize
        ...
        [~,Popselect]     = min(t(1,:));  %寻找离第三类权重向量最近的终代解
        Popkeep(Popselect) = 0;
        offA = Population(Popselect);

        offB    = rRelate(i);
        [OffBObj,~] = MeanEffective(Problem,offB);

        normW   = sqrt(sum(W(y,:).^2,2));
        normP   = sqrt(sum((OffBObj-Z).^2,2));
        CosineP = sum((OffBObj-Z).*W(y,:),2)./normW./normP;
        ...
        while guideFE > 0
            ...
            offC = OperatorGAhalf(Problem,[offA,offB]);
            [obj,~] = MeanEffective(Problem,offC);
            % obj = offC.obj;
