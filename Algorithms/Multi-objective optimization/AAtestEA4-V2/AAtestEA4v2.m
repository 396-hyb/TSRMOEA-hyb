classdef AAtestEA4v2 < ALGORITHM
        ...
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(Population)
                ...
                if length(find(flag == 0)) == 0 || Problem.FE >= Problem.maxFE  %所有小区域的指标都小于lambda，则开始切换。
                    ...
                    xNR     = find(Reta > eta);  %非鲁棒解
                    PopR    = Population(xR);  % 为一行多列
                    PopV3   = Population(xNR);
                    ...
                    if length(find(flagV3 == 1)) > 0
                        if ~isempty(rRelateV3)
                            if length(find(flagV3 == 1)) == 1
                                ...
                                populationThree = DeGuideThree(Problem, PopV3, flagV3, W, Z, eta);
                            end
                        end
                    end
