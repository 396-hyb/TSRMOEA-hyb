classdef AAtestEA6eta10 < ALGORITHM
...
% 在第一阶段，每代解都要判断是否存档
% 第三类用平均目标值选择解
% 去掉引导，直接用平均目标值计算pbi代替
    methods
        function main(Algorithm,Problem)
            
            %% Parameter setting
            [eta, ArchGEN, gap, lambda] = Algorithm.ParameterSet(0.2, 30, 30, 1e-3);
            ...
            while Algorithm.NotTerminated(Population)
                ...
                if length(find(flag == 0)) == 0 || Problem.FE >= Problem.maxFE * 0.9  %所有小区域的指标都小于lambda，则开始切换。
                    ...
                    % [~,FrontNo,CrowdDis] = EnvironmentalSelection(Population,Problem.N,PopObjV);
                    ...
                    while remainGen > 0
                        ...
                        % for i = 1 : Problem.N
                        %     P = B(i,randperm(size(B,2)));
                        %     Offspring = OperatorGAhalf(Problem, Population(P(1:2)), {1,10,1,50});
                        %     [OffObj,~] = MeanEffective(Problem,Offspring);
                        %     Z = min(Z,OffObj);
                        %     normW   = sqrt(sum(W(P,:).^2,2));
                        %     normP   = sqrt(sum((OffObjV(P,:)-repmat(Z,T,1)).^2,2));
                        %     normO   = sqrt(sum((OffObj-Z).^2,2));
                        %     CosineP = sum((OffObjV(P,:)-repmat(Z,T,1)).*W(P,:),2)./normW./normP;
                        %     CosineO = sum(repmat(OffObj-Z,T,1).*W(P,:),2)./normW./normO;
                        %     g_old   = normP.*CosineP + 5*normP.*sqrt(1-CosineP.^2);
                        %     g_new   = normO.*CosineO + 5*normO.*sqrt(1-CosineO.^2);
                        %     replaceIndex = P(g_old>=g_new);
                        %     Population(replaceIndex) = Offspring;
                        %     OffObjV(replaceIndex,:) = repmat(OffObj,length(replaceIndex),1);
                        % end
                        remainGen = remainGen - 1;
                    end
