...
%--------------------------------------------------------------------------
    ...
    for i = 1 : length(guidePop)
        PopX         = Problem.Perturb(guidePop(i).dec); 
    ...
    while guideFE > 0
        ...
        for i = 1 : len
            ...
            OffX    = Problem.Perturb(Offspring.dec); 
            OffObjV = mean(OffX.objs,1);   
            ...
            normP   = sqrt(sum((PopObjV-repmat(Z,T,1)).^2,2));
            normO   = sqrt(sum((OffObjV-Z).^2,2));
            CosineP = sum((PopObjV-repmat(Z,T,1)).*WG(P,:),2)./normW./normP;
            CosineO = sum(repmat(OffObjV-Z,T,1).*WG(P,:),2)./normW./normO;
            ...
            % if RE <= eta
                C = P(g_old>=g_new);
                guidePop(C) = Offspring;
                % A(C, :) = repmat(B, length(C), 1);
                PopObjV(C,:)  = repmat(OffObjV, length(C), 1) ;
